...
public final class StdAudio {
    ...
    /**
     * Reads audio samples from a file (in .wav or .au format) and returns them as a
     * double array with values between -1.0 and +1.0. The audio file must be 16-bit
     * with a sampling rate of 44,100. It can be mono or stereo.
     *
    ...
    public static double[] read(String filename) {
        ...
        if (audioFormat.getSampleRate() != SAMPLE_RATE) {
            throw new IllegalArgumentException("StdAudio.read() currently supports only a sample rate of " + SAMPLE_RATE
                    + " Hz\n" + "audio format: " + audioFormat);
        }

        // require 16-bit audio
        if (audioFormat.getSampleSizeInBits() != BITS_PER_SAMPLE) {
            throw new IllegalArgumentException("StdAudio.read() currently supports only " + BITS_PER_SAMPLE
                    + "-bit audio\n" + "audio format: " + audioFormat);
        }
        ...
        // little endian, stereo
            ...
            for (int i = 0; i < n / 4; i++) {
                double left = ((short) (((bytes[4 * i + 1] & 0xFF) << 8) | (bytes[4 * i + 0] & 0xFF)))
                        / ((double) MAX_16_BIT);
                double right = ((short) (((bytes[4 * i + 3] & 0xFF) << 8) | (bytes[4 * i + 2] & 0xFF)))
                        / ((double) MAX_16_BIT);
        ...
        // To do handle big endian (or other formats)
        else
